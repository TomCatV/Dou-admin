...
- 圈主后台：给付费圈主使用，圈主主账号默认管理绑定用户名下全部活跃圈子，子账号和只读账号继续管理单个授权圈子。
...
| 圈主账号 | 由超级管理员或授权管理员派发，绑定小程序注册用户，登录名固定为 `dxq_id` | 绑定用户名下全部活跃圈子 |
...
- 圈主主账号默认访问绑定用户名下全部活跃圈子，子账号/只读账号继续访问单个被授权 `circle_id`；所有查询必须强制带租户范围过滤。
- 圈主查看主房间全部聊天记录不能默认开放，必须由超级管理员先创建账号分组，再对分组开启「主房间聊天记录查看」授权；只有该分组内的圈主账号可以查看自己绑定圈子的主房间聊天记录。
...
- 所有 SQL 必须按账号类型追加租户范围过滤：`tenant_owner` 允许 `bound_user_id` 名下全部活跃圈子，`tenant_staff/tenant_viewer` 继续使用单个 `scope_circle_id`。
...
- 圈主账号派发：远程搜索已注册小程序用户、选择套餐、生成初始密码；主账号自动经营该用户名下全部活跃圈子，子账号/只读账号仍需选择授权圈子。
- 圈主子账号数量限制。
...
这是一个单独的隐私敏感能力，不能混在普通 `circle:content:view` 里。

### 授权流程

1. 超级管理员进入账号分组。
2. 创建分组，例如「高级版圈主」「托管运营客户」「内测圈主」。
3. 将指定圈主后台账号加入分组。
4. 对该分组开启「主房间聊天记录查看」授权。
5. 组内账号登录圈主后台后，只能查看自己绑定圈子的主房间聊天记录。

### 安全边界

- 未加入授权分组的圈主账号无权查看。
- 分组停用后，组内账号立即失去查看能力。
- 平台超级管理员仍可全局查看，但需要写审计日志。
- 圈主账号只能查看 `scope_circle_id` 对应圈子的主房间，不能传其他 `circle_id` 越权查看。
- 每次查看主房间聊天记录都要写审计日志，记录账号、圈子、房间、分页和搜索关键词。
...
- `POST /api/admin/account-groups/:id/members`
- `DELETE /api/admin/account-groups/:id/members/:adminUserId`
- `GET /api/admin/circles/:id/main-room/messages`
