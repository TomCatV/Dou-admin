...
| AI 能力 | OpenAI 官方文档 | P5 模型选择、提示词升级、数据隐私、成本控制和回退策略 |
...
| --- | --- | --- | --- |
| P0 | 商品中心、店铺资料、通知跳转和全角色验收矩阵已具备实现级设计 | 允许按本文继续开发 P0 剩余子项 | 开发店铺资料页、通知跳转筛选和 P0 验收脚本 |
| P1 | 只有产品目标，缺少公开接口、H5 页面和订单草稿细节 | 不允许 | 先写 `P1 商品中心与 H5 商品页设计` |
| P2 | 只有支付方向和官方文档索引，缺少支付意图、回调、查单、关单、对账设计 | 不允许 | 查微信/支付宝官方文档后写 `P2 扫码支付技术设计` |
| P3 | 有售后资金方向，缺少风控、平台介入、补发换卡密和对账细节 | 不允许 | 写 `P3 售后风控资金闭环设计` |
| P4 | 有运营方向，缺少活动/渠道/分销/归因模型 | 不允许 | 写 `P4 运营工具与分销设计` |
| P5 | 有 AI 方向，缺少数据边界、模型策略、成本和人工确认点 | 不允许 | 查 AI 官方文档后写 `P5 AI 经营助手设计` |

当前执行顺序：

1. 继续补 P0 文档，先把店铺资料、通知跳转和全角色验收矩阵写到能直接开发。
2. 按 P0 文档完成 P0 剩余代码和回归。
3. P0 验收后再写 P1 详细设计，不跨阶段抢写支付代码。
4. P1 文档通过后再进入 H5 商品页开发。
5. P2 之前必须完成微信和支付宝官方支付文档复核、支付状态机、回调验签、查单关单和对账设计。
...
1. PATCH 必须 `requireAdminPermission('circle:content:view')` 或后续独立 `tenant:store:manage`。
