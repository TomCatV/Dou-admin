...
| 支付宝电脑网站支付 | https://opendocs.alipay.com/open/repo-0038oa、https://opendocs.alipay.com/apis/api_1/alipay.trade.page.pay | H5 PC 端直达支付使用 `alipay.trade.page.pay` 标准收银台跳转，`biz_content.product_code=FAST_INSTANT_TRADE_PAY`；默认不传 `qr_pay_mode` / `qrcode_width`，避免进入嵌入式二维码页 |
...
5. 超时关单：支付意图过期后关闭第三方待支付交易，未付款本地订单保留 `created`，允许买家重新生成二维码。
...
新增迁移：`036_commerce_payment_intents.sql`
...
draft -> order_created -> paying -> paid -> delivered
                         -> intent_closed -> paying
                         -> unknown -> paid/intent_closed/failed
```
...
2. 回调金额必须等于本地订单金额。
3. 重复回调返回成功但不重复交付。
4. 支付意图过期后先查单，未支付再关闭当前第三方交易；关闭支付意图不等于取消本地订单。
5. 商品改价后已有支付意图不改金额；需要关闭旧意图并重新下单或按后端校验重新生成。
...
| `/api/shop/payment-intents/:id/close` | POST | 用户取消当前二维码或超时关闭当前支付意图 |
...
1. 选择支付方式后请求支付意图。
2. 微信 Native 和支付宝当面付预创建展示站内二维码、倒计时、订单金额和商品标题。
3. 支付宝电脑网站支付在 PC H5 端直接跳转支付宝标准收银台，由支付宝页面展示订单信息和扫码区域；不使用 `qr_pay_mode=4` 的嵌入式二维码模式。
4. 每 3-5 秒轮询本地支付意图，不直接信任第三方前端跳转。
5. 倒计时结束调用同步状态；未支付则提示重新生成二维码。
6. 支付成功跳转取货页。
...
5. 支付超时或用户取消后只关闭当前支付意图，本地未付款订单可重新生成二维码。
6. 查单能恢复回调丢失的成功订单。
...
1. Dou-Server 新增 `036_commerce_payment_intents.sql`，补齐 `commerce_payment_intents`、`commerce_payment_logs`，并给 `orders` 增加 `pay_channel`、`payment_intent_id`、`closed_at`。
2. H5 匿名买家通过下单联系方式生成稳定的 `h5_buyer_*` 本地用户，订单、资源购买、卡密发放和圈主钱包结算复用现有资源卡链路。
3. 新增 `/api/shop/order-drafts/:draftId/orders`、`/api/shop/orders/:orderId/payment-intents`、`/api/shop/payment-intents/:id`、`/api/shop/orders/:orderId/payment-status`、`/api/shop/payment-intents/:id/close`。
4. 微信 Native 支付实现创建二维码、查单、关单、支付通知；支付宝当面付实现预创建、查单、关单、异步通知。开发环境支持 mock，生产环境仍必须配置真实证书、公钥和回调地址。
5. Dou-Admin 公开确认订单页会在本地生成二维码，不把支付串发送到第三方二维码服务；轮询后端支付意图，支付成功后进入订单页展示资源链接或卡密。
6. 订单公开查询继续要求“订单号 + 下单联系方式”，已支付后才返回交付私密字段。
7. 支付成功推进前会校验微信/支付宝回传金额与本地支付意图一致；微信回传商户号、支付宝回传 AppID 与本地配置不一致时不交付，进入异常日志。
8. 查单返回 `NOTPAY` / `WAIT_BUYER_PAY` 时保持原二维码可用状态，不把待支付误判为失败；仅支付成功、关闭、金额异常等可信状态改变订单。
