...
function productLinkWith(params: Record<string, string>) {
  const query = new URLSearchParams(params);
  return `${shopBaseUrl()}/#/shop/product/{商品KEY}?${query.toString()}`;
}
...
async function createCoupon() {
  await tenantConversionApi.createCoupon({
    ...
    discount_value: Math.round(
      Number(couponForm.discount_value_yuan || 0) * 100
    ),
...
<template>
  <div v-loading="loading" class="tenant-page">
    ...
    <div class="panel">
      <h2>新人欢迎自动化</h2>
      <el-switch
        v-model="welcome.enabled"
        active-text="启用"
        inactive-text="停用"
      />
      <el-input
        v-model="welcome.message"
        class="mt"
        type="textarea"
        :rows="3"
        placeholder="欢迎语"
      ...
      <el-input
        v-model="welcome.coupon_code"
        class="mt"
        placeholder="关联优惠券编码"
      />
      <el-button class="mt" type="primary" @click="saveWelcome"
        >保存自动化</el-button
      >
    </div>
    ...
    <el-row :gutter="16" class="mt">
      <el-col :xs="24" :lg="8">
        <div class="panel">
          ...
          />
          <el-input
            v-model="couponForm.discount_value_yuan"
            class="mt"
            placeholder="优惠金额，元"
          />
          <el-input
            v-model="couponForm.min_amount_yuan"
            class="mt"
            placeholder="最低消费，元"
          />
          <el-input-number
            ...
            :min="0"
          />
          <el-button class="mt full" type="primary" @click="createCoupon"
            >创建优惠券</el-button
          >
            ...
            <el-table-column label="链接" width="80">
              <template #default="{ row }">
                <el-button
                  link
                  type="primary"
                  @click="copyText(productLinkWith({ coupon: row.code }))"
                  >复制</el-button
                >
              </template>
            </el-table-column>
      ...
      <el-col :xs="24" :lg="8">
        <div class="panel">
          ...
          />
          <el-input
            v-model="inviteForm.reward_text"
            class="mt"
            placeholder="奖励说明"
          />
          <el-button class="mt full" type="primary" @click="createInvite"
            >创建邀请码</el-button
          >
            ...
            <el-table-column label="链接" width="80">
              <template #default="{ row }">
                <el-button
                  link
                  type="primary"
                  @click="copyText(productLinkWith({ invite: row.code }))"
                  >复制</el-button
                >
              </template>
            </el-table-column>
      ...
      <el-col :xs="24" :lg="8">
        <div class="panel">
          ...
          <el-input
            v-model="campaignForm.goal_text"
            class="mt"
            placeholder="目标"
          />
          <el-button class="mt full" type="primary" @click="createCampaign"
            >创建活动</el-button
          >
